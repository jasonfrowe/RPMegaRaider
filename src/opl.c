...
void update_music() {
    ...
    if (music_wait_ticks == 0) {
        while (music_wait_ticks == 0) {
            ...
            // Some exported tracks begin with a one-tick key-off packet (e.g. B0 00 01 00).
            // Replaying that exact packet at loop wrap can create an audible hiccup.
